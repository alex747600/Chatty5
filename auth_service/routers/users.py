...
@router.post("/register", summary="Регистрация пользователя", description="Создает нового пользователя с указанным именем и паролем.", tags=["Пользователи"])
...
@router.post("/add-mail", summary="Запрос на добавление email", description="Отправляет ссылку на указанный email для его привязки к аккаунту.", tags=["Управление email"])
async def request_add_email(email_data: schemas.EmailAdd, user: models.User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    if user.email:
    ...
    confirmation_url = f"http://localhost/users/confirm-email?token={token}"
    send_email(email_data.email, "Подтверждение добавления email", f"Перейдите по ссылке для добавления email: {confirmation_url}")
...
@router.post("/del-mail", summary="Запрос на удаление email", description="Отправляет ссылку на текущий email для его отвязки от аккаунта.", tags=["Управление email"])
async def request_delete_email(user: models.User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    if not user.email:
    ...
    deletion_url = f"http://localhost/users/delete-email?token={token}"
    send_email(user.email, "Подтверждение удаления email", f"Перейдите по ссылке для удаления email: {deletion_url}")
...
@router.post("/del-user", summary="Запрос на удаление аккаунта", description="Отправляет ссылку на email для удаления аккаунта, если email привязан и активен. Иначе требует пароль.", tags=["Пользователи"])
async def delete_user(password: schemas.PasswordConfirm | None = None, user: models.User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    if user.email and user.is_active:
        token = create_email_token({"sub": user.username, "action": "delete_user"})
        deletion_url = f"http://localhost/users/delete-user?token={token}"
        send_email(user.email, "Удаление аккаунта", f"Перейдите по ссылке для удаления аккаунта: {deletion_url}")
...
@router.patch("/user-edit", summary="Редактирование профиля", description="Позволяет изменить имя пользователя, если новое имя не занято.", tags=["Пользователи"])
async def edit_user(user_data: schemas.UserEdit, user: models.User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
@router.post("/internal/verify-token", include_in_schema=False)
async def internal_verify_token(credentials: HTTPAuthorizationCredentials = Depends(HTTPBearer())):
    try:
        payload = jwt.decode(
